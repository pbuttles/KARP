...
class ScpiProperty:
    ...
    def setwrapper(self,event):
        ...
        if str(self.command) == "b'SOUR1:VOLT'" or str(self.command) == "b'SOUR2:VOLT'":
            if float(self.value.get()) >= 120.0 or float(self.value.get()) <= -20.0:
                if float(self.value.get()) >= 200.0 or float(self.value.get()) <= -200.0:
    ...
    def update(self):
        ...
        if self._port is not None:
            if (self._port.name in (i.device for i in ports)) and not self.needs_reset:
                ...
                MainGui.status_box1.configure(background='red')
                time.sleep(0.05)
                return True
        else:
    ...
    def update(self):
        ...
        if self._port is not None:
            if (self._serial_number in (rm.open_resource(i).resource_info.resource_name.split("::")[3] for i in self.usb_ports)) and not self.needs_reset:
                ...
                MainGui.status_box2.configure(background='red')
                time.sleep(0.05)
                return True
        else:
            if self._serial_number is not None:
...
class MainGui:
    ...
    def main_task(self):
        ...
        def animate(self):
            self.xval.append(self.indvar.value.get())
            self.yval.append(self.depvar.value.get())
    ...
    def build_main_window(self):
        ...
        def startrecord(event):
            ...
            lims = [[-1,2],[-20,120],[0,100],[-20,120],[-20,120],[-20,100],[-1,2],[-20,120],[0,100],[-20,120],[-20,120],[-20,100],[-20,100],[-20,100]]
            ...
            datalabels=["Output Relay 1","Target Voltage 1 (V)","Slew Rate 1 (V/s)","Output Voltage 1 (V)","Measured Voltage 1 (V)","Measured Current 1 (A)","Output Relay 2","Target Voltage 2 (V)","Slew Rate 2 (V/s)","Output Voltage 2 (V)","Measured Voltage 2 (V)","Measured Current 2 (A)","Primary Keysight Measurement","Secondary Keysight Measurement","Keysight Test Signal"]
        ...
        self.status_box1 = Label(frame, text="UNCONFIGURED", relief=SUNKEN)
        ...
        frame = Frame(tab1, border=2, relief=GROOVE)
        frame.grid(row=1, column=2, sticky="WE", padx=10, pady=5)
        frame.grid_columnconfigure(3, weight=2)
        ...
        Label(frame, text="Connection Status:").grid(row=1, column=2, sticky="E", padx=(30, 1))
        Label(frame, text="Connected To (*IDN?):").grid(row=2, column=2, sticky="E", padx=(30, 1))
        self.status_box2 = Label(frame, text="UNCONFIGURED", relief=SUNKEN)
        ...
        for channel in (1, 2):
            frame = Frame(tab1, border=2, relief=GROOVE)
            frame.grid(row=2+channel, padx=10, pady=5, column=1, sticky=NSEW)
        ...
        prop = ScpiPropertyFloat(frame, 2, self.usb_port, ":FETCh:IMPedance:FORMatted?", ["Capacitance (F) / Primary Measurement","Resistance (Ω) /  Secondary Measurement"], can_set=False)
        self._scpi_properties.append(prop)
        
        """ Generate the Live Plotting graph """
        plotframe = Frame(tab1, border=2, relief=GROOVE)
        plotframe.grid(row=3,column=2,columnspan=1,rowspan=2, padx=10, pady=10)
        self.fig = plt.Figure()
